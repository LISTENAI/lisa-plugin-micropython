...
type ParseArgField =
  | ParseArgFieldBase
  | ParseArgFieldWithArg
  | ParseArgFieldWithOptionalArg;

export type ParseArgOptions = Record<string, ParseArgField>;

type ParseArgResult<Fields> = {
  [Key in keyof Fields]?: Fields[Extract<
    keyof Fields,
    Key
  >] extends ParseArgFieldWithOptionalArg
    ? string | boolean
...
export default function parseArgs<T = ParseArgOptions>(
